...
function getC2PMask(wsrc, hsrc, wdst, hdst, xcenter, ycenter, lpadding, rpadding, rmax, alpha)
   ...
   buildMask(mask, xcenter, ycenter, kr, ktheta, alpha)
   if lpadding > 0 then
...
function getP2CMask(wsrc, hsrc, wdst, hdst, xcenter, ycenter, rmax, alpha)
   ...
   buildMask = inline.load [[
	 ...
	 float invalpha = lua_tonumber(L, 7) * 0.5f;
	 ...
	 for (i = 0; i < hdst; ++i) {
	    for (j = 0; j < wdst; ++j) {
	       x = (float)j - xcenter;
	       y = (float)i - ycenter;
	       mask_p[      + i*ms[1] + j*ms[2] ] = pow(x*x+y*y, invalpha)*ky;
	       mask_p[ms[0] + i*ms[1] + j*ms[2] ] = fmod(atan2(y, x) + pi2, pi2)*kx;
	    }
	 }
   ]]
   buildMask(mask, xcenter, ycenter, kx, ky, pi2, 1.0/alpha)
   return mask
end
...
function cartesian2polar_testme()
   ...
   image.display{image=cartesian2polar(im, mask2), min=0, max=1}
